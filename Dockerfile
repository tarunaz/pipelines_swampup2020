...
RUN curl $REGISTRY/tarun-libs-release-local/com/jfrog/backend/1.0.0/backend-1.0.0.jar --output server1.jar
RUN curl $REGISTRY/tarun-npm-dev-local/frontend/-/frontend-3.0.0.tgz --output client2.tgz

#Extract vue app
RUN tar -xzf client2.tgz && rm client2.tgz
